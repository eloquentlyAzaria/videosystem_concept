...
-------------------------------------------------
revised by @iamfreddiegibbs on discord
...
class HeaderBar(ctk.CTkFrame):
    def __init__(self, master, on_search: Callable[[str], None]):
        ...
        self.avatar = ctk.CTkButton(self, text="A", width=36, height=36,
                                    corner_radius=18, fg_color="#3d3d3d", hover=False)
...
class App(ctk.CTk):
    ...
    def toggle_sidebar(self):
        ...
        self.manual_override = True   # prevent resize from fighting toggle
    ...
    def _open_video(self, video: Video):
        """Open a video overlay player when a thumbnail is clicked."""
        PlayerOverlay(self, video)

    def _on_resize(self, event):
        """Automatically collapse/expand sidebar based on window size."""
        if self.manual_override:
            return  # let manual toggle win
